...
The map function goes through every single element of a object(list or vectors) and applies a passed in function to each element. There is one VERY important point to pay attention to is that the .map function goes through EVERY 👏 SINGLE 👏 ELEMENT 👏 and returns a brand new list with the modified values.
...
Just like map, the filter function goes through EVERY 👏 SINGLE 👏 ELEMENT 👏 and returns a new list  with just the elements that didn't get filtered out.
